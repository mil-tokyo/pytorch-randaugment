...
def get_dataloaders(dataset, batch, dataroot, split=0.15, split_idx=0):
    ...
    if C.get()['aug'] == 'randaugment':
        transform_train.transforms.insert(place_to_insert, RandAugment(C.get()['randaug']['N'], C.get()['randaug']['M'], not ('imagenet' in dataset),fillcolor = fillcolor))
    ...
    trainloader = torch.utils.data.DataLoader(
        total_trainset, batch_size=batch, shuffle=True if train_sampler is None else False, num_workers=64, pin_memory=False,
        sampler=train_sampler, drop_last=True)
    validloader = torch.utils.data.DataLoader(
        total_trainset, batch_size=batch, shuffle=False, num_workers=64, pin_memory=False,
